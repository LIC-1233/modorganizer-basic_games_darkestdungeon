...
class DarkestDungeonGame(BasicGame, mobase.IPluginFileMapper):
    ...
    def Refreshed(self):
        pass
        def acf_parser(acf_file:str)->Dict[str,Dict[str,Dict[str,Dict[str,str]]]]:
        ...
        qInfo("refreshing")
        acf_path = find_games()["262060"].parent.parent/"workshop"/"appworkshop_262060.acf"
        ...
        mo_workshop_PublishedFileId:Dict[str,mobase.IModInterface] = {str(i.stem.strip("w")):modlist.getMod(str(i.parent.parent.name)) for i in mod_root_folder.glob("*/project_file/w*.manifest")}
        mo_local_PublishedFileId:Dict[str,mobase.IModInterface] = {str(i.stem.strip("l")):modlist.getMod(str(i.parent.parent.name)) for i in mod_root_folder.glob("*/project_file/l*.manifest")}
        ...
        for mod_name in mod_names:
            ...
            if not mod.version().isValid():
                qInfo(f"Setting version of {ascii(mod_name)} to {mod_data.mod_versions}")
            ...
            if not mod.url() and manifest_file.exists() and manifest_file.name.startswith("w"):
                qInfo(f"Setting url of {ascii(mod_name)} to {mod_data.mod_PublishedFileId}")
                mod.setUrl(f"https://steamcommunity.com/sharedfiles/filedetails/?id={mod_data.mod_PublishedFileId}")
        ...
        workshop_path = find_games()["262060"].parent.parent/"workshop"/"content"/"262060"
        for PublishedFileId in set([i for i in workshop_items.keys()]) - set([i for i in mo_workshop_PublishedFileId.keys()]):
            mod_title = xml_data.mod_xml_parser(workshop_path / PublishedFileId / "project.xml").mod_title
            qInfo(f"Adding mod {ascii(mod_title)}")
            if mod_title:=copy_local_mod(workshop_path / PublishedFileId, mod_root_folder / mod_title):
                ...
                if preview_file.exists():
                    ...
                    preview_file.rename(mo_mod_folder / "preview_file" / (PublishedFileId+".png"))
                if txt_file.exists():
                    txt_file.unlink()
                if log_file.exists():
                    log_file.unlink()
                ...
                if xml_file.exists():
                    xml_file.rename(mo_mod_folder / "project_file" / (PublishedFileId+".xml"))
                    open(mo_mod_folder / "project_file" / ("w"+PublishedFileId+".manifest"), "w+").write(workshop_items[PublishedFileId]["manifest"])
            else:
        ...
        for PublishedFileId in set([i for i in workshop_items.keys()]) & set([i for i in mo_workshop_PublishedFileId]):
            ...
            old_manifest_file = Path(mo_workshop_PublishedFileId[PublishedFileId].absolutePath()) / "project_file" / ("w"+PublishedFileId+".manifest")
            old_manifest = old_manifest_file.read_text() if old_manifest_file.exists() else ""
            if not old_manifest:
                qInfo(mo_workshop_PublishedFileId[PublishedFileId].name()+" manifest is empty")
            if new_manifest != old_manifest:
                mo_workshop_PublishedFileId[PublishedFileId].setNewestVersion(mobase.VersionInfo(*xml_data.mod_xml_parser(workshop_path / PublishedFileId / "project.xml").mod_versions))
        ...
        for xml_file in (Path(self.gameDirectory().absolutePath()) / "mods").glob("*/project.xml"):
            ...
            if not manifest_files or not (set([i.stem.strip("l") for i in manifest_files]) & set([i for i in mo_local_PublishedFileId.keys()])):
            ...
            if xml_file.exists():
                ...
                open(xml_file.parent / "project_file" / ("l"+id+".manifest"), "w+").write("")
...
<project>
	...
	<PublishedFileId>0000000000</PublishedFileId>
        ...
        modlist = [i for i in modlist if self._organizer.modList().state(i) & mobase.ModState.ACTIVE]
        for mod in modlist:
            raid_settings_file = self._get_mods_path() / mod / "scripts" / "raid_settings.json"
            if raid_settings_file.exists():
                for key in raid_settings_keys:
                    raid_settings[key]+=(json.loads(open(raid_settings_file).read())[key])
        ...
        if not (Path(self._organizer.overwritePath()) / "effects").exists():
            ...
            for file in (Path(self._organizer.overwritePath()) / "effects").glob("*.effects.darkest"):
                file.unlink()
        for mod in modlist:
            for effect_file in (self._get_mods_path() / mod / "effects").glob("*.effects.darkest"):
                effect_files[effect_file.name].append(effect_file)
        for key, files in effect_files.items():
            if len(files)>1:
                contents = "\n".join([i.read_text() for i in files])
                open(Path(self._organizer.overwritePath()) / "effects" / f"{key}","w+").write(contents)
                effect_mapping.append(mobase.Mapping(
                    str(Path(self._organizer.overwritePath()) / "effects" / f"{key}"),
        ...
        for mod in modlist:
            for dir in set(none_mod_dirs) & set([i.name for i in (self._get_mods_path() / mod).glob("*")]):
                qInfo(f"mapping {ascii(str(self._get_mods_path() / mod / dir))} to {ascii(str(self._get_game_path() / dir))}")
                nomod_mapping.append(mobase.Mapping(
